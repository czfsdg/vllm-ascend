...
by `apply_runtime_cuts`; leave it `true` to match the reference D-Cut behavior,
or set it to `false` only when you want planning/timing diagnostics without
mutating the scheduler output.
