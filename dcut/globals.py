...
# no graph_task_update for recurrent metadata). Recurrent GDN metadata is
# built on CPU from host fallback tuples, copied into stable tensors, and read
# by the kernel inside the graph. Conv1D still uses graph_task_update
# (host tuple args).
...
# before each replay. Metadata values are constructed on CPU from host tuples
# to avoid per-field NPU fill kernels; the GDN op inside the captured graph
# reads these buffers at replay time.
#
